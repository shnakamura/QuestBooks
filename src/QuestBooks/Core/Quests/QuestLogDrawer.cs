...
public class QuestLogDrawer : ModSystem
{
    ...
    {
        var display = active ?? !DisplayLog;
        var changed = display != DisplayLog;
    ...
    private static bool targetCleared;
    ...
    public static void DrawQuestLog()
    {
        if (AnimationInProgress)
        {
            if (TargetDisplayLog)
            {
                ...
                {
                    QuestLogDrawOffset = new Vector2(0f, OpenTimer);
                }

                QuestLogDrawOpacity = (OpenAnimationLength - OpenTimer) / (float)OpenAnimationLength;
                QuestLogDrawOffset = new Vector2(0f, float.Lerp(QuestLogDrawOffset.Y, 0f, 0.15f));
            }

            else
            {
                QuestLogDrawOpacity = OpenTimer / (float)OpenAnimationLength;
                QuestLogDrawOffset = new Vector2(0f, float.Lerp(QuestLogDrawOffset.Y, OpenAnimationLength - OpenTimer, 0.15f));
            }
        }
    ...
    {
        var mouseTextLayer = layers.FindIndex(layer => layer.Name.Equals("Vanilla: Mouse Text"));
        ...
        {
            var achievementPositionInInventory = new Vector2(516f, 30f).ToPoint();
            Rectangle achievement = new(achievementPositionInInventory.X, achievementPositionInInventory.Y, 48, 48);

            var iconArea = achievement.CookieCutter(new Vector2(2f, 0f), new Vector2(0.7f, 0.7f));
            var hovered = false;
