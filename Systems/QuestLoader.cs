...
internal class QuestLoader : ModSystem
{
    ...
    internal static void LoadQuests(Mod mod)
    {
        ...
        QuestManager.AvailableQuestElementTypes.AddRange
        (
            types.Where(t => !t.IsAbstract && t.IsAssignableTo(typeof(QuestLogElement)))
                .Select(t => new KeyValuePair<Type, QuestLogElement>(t, (QuestLogElement)Activator.CreateInstance(t)))
                .Select
                (kvp =>
                    {
                        kvp.Value.TemplateInstance = true;
                        return kvp;
                    }
                )
    ...
    public static void LoadCompletedQuests(IEnumerable<string> completedQuests, out IEnumerable<string> unloadedQuests)
    {
        ...
        foreach (var key in completedQuests)
        {
    ...
    {
        var serialized = JsonConvert.SerializeObject(questLog, Formatting.None, JsonTypeResolverFix.Settings);
    ...
    public static List<QuestBook> LoadQuestLog(string filePath)
    {
        ...
        try
        {
            var book = JsonConvert.DeserializeObject<List<QuestBook>>(serialized, JsonTypeResolverFix.Settings);
            return book;
        }
        catch
        {
