...
public partial class BasicQuestLogStyle : QuestLogStyle
{
    ...
    private RenderTargetBinding[] returnTargets;
    private BlendState returnBlend;
    private SamplerState returnSampler;
    private DepthStencilState returnDepth;
    private RasterizerState returnRaster;
    private Effect returnEffect;
    private Matrix returnMatrix = Matrix.Identity;

    private RenderTarget2D booksTarget;
    private RenderTarget2D chaptersTarget;
    private RenderTarget2D libraryTarget;

    private RenderTarget2D questInfoTarget;
    private RenderTarget2D previousQuestInfoTarget;

    private RenderTarget2D questAreaTarget;
    private RenderTarget2D previousQuestAreaTarget;

    private bool swipingBetweenInfoPages;
    private float questInfoSwipeOffset;
    ...
    public override void UpdateLog()
    {
        ...
        var questLogCenter = LogArea.Center();
        ...
        if (QuestLogDrawer.AnimationInProgress)
        {
            ...
            ScaledMousePos = new Vector2(-100000f, -100000f);
            MouseCanvas = new Point(-100000, -100000);
        }
            ...
            (sb =>
                {
                    ...
                    var source = pageFlipping.Frame(verticalFrames: 2, frameY: pageFlippingTimer / frameLength % sheetFrames);

                    var bookSize = QuestAssets.QuestLogCanvas.Asset.Size();
                    var bookArea = CenteredRectangle(questLogCenter, bookSize * LogScale);
        ...
        UpdateChapters
        (
            chaptersTarget.Bounds.CreateScaledMargins(top: FadeDesignation, bottom: FadeDesignation),
            (MouseCanvas - chapters.Location).ToVector2() * (chaptersTarget.Bounds.Size() / chapters.Size())
        );
        ...
        var infoPage = SelectedElement is not null && (SelectedElement.HasInfoPage || UseDesigner);

        if (infoPage)
        {
            var transform = Matrix.CreateScale(TargetScale);
            ...
            var infoMousePosition = (MouseCanvas - questInfo.Location).ToVector2() * (questInfoTarget.Bounds.Size() / questInfo.Size()) / TargetScale;
        ...
        (sb =>
            {
                ...
                var resizeScale = LogScale / TargetScale;
                books = libraryTarget.Bounds.CookieCutter(new Vector2(-0.5f, 0f), new Vector2(0.5f, 1f)).CreateMargins(right: 4);
                chapters = libraryTarget.Bounds.CookieCutter(new Vector2(0.5f, 0f), new Vector2(0.5f, 1f)).CreateMargins(4);

                if (questInfoSwipeOffset != 0f)
                {
                    var farFromEdge = Math.Abs(questInfoSwipeOffset) > libraryTarget.Height * FadeDesignation * 0.5f;
                    ...
                    var libraryOffset = -float.Sign(questInfoSwipeOffset) * (questInfoTarget.Height - Math.Abs(questInfoSwipeOffset));
                    var questInfoOffset = questInfoSwipeOffset;
                    ...
                    if (swipingBetweenInfoPages)
                    {
                        sb.Draw
                        (
                            previousQuestInfoTarget,
                            libraryTarget.Bounds.Center() + new Vector2(0f, libraryOffset),
                            null,
                            Color.White,
                            0f,
                            previousQuestInfoTarget.Size() * 0.5f,
                            1f,
                            SpriteEffects.None,
                            0f
                        );
    ...
    protected virtual bool DrawBookOpening(Vector2 questLogCenter)
    {
        ...
        var targetDrawHeight = questLogCenter.Y;
    ...
    {
        var localizationKey = $"Mods.{QuestManager.QuestLogMods[questLogKey].Name}.QuestBooks.{questLogKey}.Name";
    ...
    public static void DrawDefaultLogTitle(SpriteBatch spriteBatch, Rectangle drawArea, string title, float opacity, bool hovered, bool selected) =>
        ...
        spriteBatch.DrawOutlinedStringInRectangle
            (drawArea, FontAssets.DeathText.Value, (hovered ? Color.Yellow : Color.White) * opacity, Color.Black * opacity, title, extraScale: 1.2f, clipBounds: false, offset: new Vector2(0f, 12f));
    ...
    public override void SelectChapter(QuestChapter chapter)
    {
        ...
        void Select()
        {
            var sign = SelectedBook.Chapters.IndexOf(chapter ?? SelectedChapter) >= SelectedBook.Chapters.IndexOf(SelectedChapter) ? 1 : -1;
    ...
    {
        questArea = logArea.CookieCutter(new Vector2(0.485f, -0.055f), new Vector2(0.45f, 0.84f));
        questInfo = logArea.CookieCutter(new Vector2(-0.5025f, -0.055f), new Vector2(0.43f, 0.84f)); // The combined area of the Books and Chapters

        // Distance between books/chapters is NOT scaled
        books = questInfo.CookieCutter(new Vector2(-0.5f, 0f), new Vector2(0.5f, 1f)).CreateMargins(right: 4);
        chapters = questInfo.CookieCutter(new Vector2(0.5f, 0f), new Vector2(0.5f, 1f)).CreateMargins(4);
    }
        ...
        (sb =>
            {
                ...
                (
                    ...
                    depth ?? DepthStencilState.Default,
                    raster ?? RasterizerState.CullNone,
                    effect ?? null,
    ...
    private void SetupTargets()
    {
        ...
        CalculateLibraryRegions(basicLogArea, out var books, out var chapters, out var questArea, out var questInfo);
